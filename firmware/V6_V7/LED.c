...
bool cellDeltaLEDIndicator (void){
    ...
    ledBlinkpattern (num_yellow_blinks, 0b110, 250, 250, 750, 500);
...
bool cellVoltageLEDIndicator (void){
    ...
    if (wait_count < 5){
        ...
        if (cellstats.mincell_mV >= MIN_DISCHARGE_CELL_VOLTAGE_mV){
        num_green_blinks = (uint8_t) ( (cellstats.mincell_mV-MIN_DISCHARGE_CELL_VOLTAGE_mV) / ((MAX_CHARGE_CELL_VOLTAGE_mV-MIN_DISCHARGE_CELL_VOLTAGE_mV)/6) + 1);      //Sligthly altered if different charging min/max voltages are set. For standard case 3000mv/4200mV: Min cell < 3.2V = 1 blinks, mincell < 3.4 = 2 blink, min cell < 3.6V = 3 blinks, min cell < 3.8V = 4 blinks, min cell < 4V = 5 blinks, mincell < 4.2V = 6 blinks.
        }
        else{
            num_green_blinks = 1;
        }
    ...
    LED_code_cycle_counter.enable = true;
    ledBlinkpattern (num_green_blinks, 0b010, 250, 250, 500, 500);
    if (LED_code_cycle_counter.value > 1){
        resetLEDBlinkPattern();
        wait_count = 0;
